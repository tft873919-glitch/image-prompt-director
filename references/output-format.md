...
- `reference_images` 可为空；如有参考图，每张图只设一个主要角色并明确优先级，数组顺序就是上传顺序。
...
## 参考图分工
| 上传顺序 | 参考图 | 路径/附件 | 优先级 | 主要角色 | 学习项 | 保留锚点 | 禁用项 |
| --- | --- | --- | --- | --- | --- | --- | --- |

## 本版不再引用
- 无 / [旧图名称](路径或附件链接)：不再引用原因
...
每版在对话中使用下面的最小交付格式；本地图片使用绝对路径的 Markdown 链接，序号就是上传顺序：

```markdown
- Prompt：[v1.2.0](Prompt绝对路径)
- 记录：[项目记录](记录绝对路径)
- 参考图：
  1. [人物身份参考](图片绝对路径) — high｜身份锁定
  2. [构图参考](图片绝对路径) — medium｜只学构图
- 本版不再引用：[旧风格图](旧图绝对路径)；无则写“无”
```
...
- `reference_images` 顺序与上传顺序一致。版本交付时在 Prompt 与记录链接之后列出本版参考图的可点击链接、优先级和作用，并注明本版不再引用的旧图；默认不展开图片预览。
- 文件无跨版本引用或差异补丁。
